...
def dashboard() -> str:
    ...
    provider_mode = "real" if os.getenv("GITHUB_TOKEN") else "public"
    gateway_summary = (
        "DuckDB embebida en /app/data/case09.duckdb, "
        "auth por X-API-Key, proveedor GitHub API."
    )
    request_rows_html = render_table_rows(requests_rows, request_columns)
    error_rows_html = render_table_rows(error_rows, error_columns)
    top_repo_rows_html = render_table_rows(top_repo_rows, top_columns)
    ...
    <head>
      ...
      <style>
        ...
        linear-gradient(135deg, #020617, #0f172a 55%, #1e293b); color:
        var(--text); }}
        .wrap {{ max-width: 1280px; margin: 0 auto; padding: 24px; }}
        .hero {{ display: grid; gap: 12px; margin-bottom: 24px; }}
        .grid {{ display: grid; grid-template-columns: repeat(auto-fit,
        minmax(220px, 1fr)); gap: 16px; margin-bottom: 24px; }}
        .card {{ background: rgba(15, 23, 42, 0.82); border: 1px solid
        var(--border); border-radius: 16px; padding: 16px; box-shadow: 0
        14px 30px rgba(0, 0, 0, 0.24); }}
    ...
    <body>
      <div class="wrap">
        <section class="hero">
          ...
          <div class="muted">{gateway_summary}</div>
        </section>
        <section class="grid">
          <div class="card">
          ...
          <div class="card">
            <div class="muted">Succeeded</div>
            <div class="metric">{stats['succeeded']}</div>
          </div>
          <div class="card">
            <div class="muted">Failed</div>
            <div class="metric">{stats['failed']}</div>
          </div>
          <div class="card">
            <div class="muted">Provider mode</div>
            <div class="metric">{provider_mode}</div>
          </div>
        </section>
        <section class="card">
        ...
        <section class="card" style="margin-top:16px;">
          ...
          <div class="table-wrap">
            <table>
              ...
              <tbody>{top_repo_rows_html}</tbody>
            </table>
          </div>
        </section>
      </div>
