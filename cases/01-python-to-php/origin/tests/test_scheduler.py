...
def test_should_publish_logic():
    ...
    # Post futuro
    future_post = Post(id="f", text="f", scheduled_at=now + timedelta(hours=1))
    assert future_post.should_publish(now) is False

    # Post pasado
    past_post = Post(id="p", text="p", scheduled_at=now - timedelta(hours=1))
    assert past_post.should_publish(now) is True

    # Post ya publicado
    already_published = Post(
        id="a", text="a", scheduled_at=now - timedelta(hours=1), published=True
    )
