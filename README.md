...
[![Version](https://img.shields.io/badge/version-4.8.0-green.svg)](https://github.com/vladimiracunadev-create/social-bot-scheduler/releases)
...
| **13** | `Node + Kafka` | **n8n** | `Go consumer` | 📊 **ClickHouse** | `case13` | [📐 Ver](cases/13-node-to-go-kafka/ARCHITECTURE.md) |
| **14** | `Next.js 15` | **n8n** | `Supabase (PostgREST)` | 🟢 **Postgres + RLS** | `case14` | [📐 Ver](cases/14-nextjs-to-supabase/ARCHITECTURE.md) |
...
> [!TIP]
> **Lote 1** (`v4.5.0`): 11, 16, 17. **Lote 2** (`v4.6.0`): 10, 12, 18. **Lote 3** (`v4.7.0`): 15, 20. **Lote 4** (`v4.8.0`): 13, 14. **19 de 20 casos operativos** vía `docker-compose --profile caseNN up -d`. Puertos por la regla `8080 + id` (ver [docs/PORTS.md](docs/PORTS.md)).
...
> [!NOTE]
> El **caso 19** tiene el código completo (bug de arranque AOT ya corregido) pero queda pendiente de verificación end-to-end. Ver [docs/PLANNED_CASES.md](docs/PLANNED_CASES.md) y [ROADMAP.md](ROADMAP.md).
...
5. **Persistencia Políglota**: 17 motores de bases de datos distintos.
