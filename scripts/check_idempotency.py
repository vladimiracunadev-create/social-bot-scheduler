...
if __name__ == "__main__":
    ...
    if action == "check" and len(sys.argv) == 4:
        ...
        print(
            json.dumps({"exists": exists, "fingerprint": fingerprint, "case": case_id})
        )
        ...
        print(
            json.dumps({"added": success, "fingerprint": fingerprint, "case": case_id})
        )
        ...
        print(
            json.dumps(
                {"error": f"Invalid action '{action}'. Use: check, add, stats, cleanup"}
            )
        )
        sys.exit(1)
