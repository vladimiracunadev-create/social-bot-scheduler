...
def validate_workflow(file_path):
    ...
    try:
        ...
        http_node = next((n for n in data["nodes"] if n["name"] == "HTTP Request"), None)
        if http_node:
            options = http_node.get("parameters", {}).get("options", {})
            if not options.get("retryOnFail"):
                print(f"[WARNING] {file_path}: 'HTTP Request' no tiene habilitado 'retryOnFail'")
                return False
...
def main():
    ...
    for i in range(1, 9):
        ...
        case_folder = next((f for f in os.listdir(cases_dir) if f.startswith(case_num)), None)
        if not case_folder:
            continue
